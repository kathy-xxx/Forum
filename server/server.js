...
function promptForDatabaseDetails() {
  rl.question("Enter database username: ", (username) => {
    rl.question("Enter database password: ", (password) => {
      rl.question("Enter database name: ", (database) => {
        tryDatabaseConnection({ username, password, database });
      });
    });
  });
}

function tryDatabaseConnection({ username, password, database }) {
  ...
  db.connect((err) => {
    if (err) {
      console.error("Database connection failed. Please try again.");
      console.error(err.message);
      promptForDatabaseDetails();
    } else {
...
function startServer() {
  ...
  app.get("/user/:id/activity", (req, res) => {
    ...
    db.query(sql, [userId], (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ message: "Error fetching user activity" });
      ...
      if (categories && categories.length > 0) {
        ...
        db.query(sqlInsertCategories, [values], (err) => {
          if (err)
            return res
              .status(500)
              .json({ error: "Error assigning categories" });
  ...
  app.post("/thread/:id/posts", (req, res) => {
    ...
    const sql =
      "INSERT INTO Post (content, thread_id, user_id) VALUES (?, ?, ?)";
