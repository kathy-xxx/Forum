...
app.get('/communities', (req, res) => {
    ...
    db.query(sql, (err, result) => {
        if (err) return res.status(500).json({ message: "Error fetching communities" });
        return res.json(result);
    });
});
...
app.get('/user/:id/activity', (req, res) => {
    ...
    db.query(sql, [userId], (err, result) => {
        if (err) return res.status(500).json({ message: "Error fetching user activity" });
...
app.post('/user/:id/join', (req, res) => {
    ...
    db.query(sql, [userId, community_id], (err, result) => {
        if (err) return res.status(500).json({ message: "Error joining community" });
...
app.get('/community/:id/threads/filter', (req, res) => {
    ...
    db.query(sql, [communityId, category], (err, result) => {
        if (err) return res.status(500).json({ error: "Error filtering threads by category" });
        return res.json(result);
    });
});
    ...
    db.query(sqlInsertThread, [title, communityId, user_id], (err, result) => {
        ...
        if (categories && categories.length > 0) {
            ...
            db.query(sqlInsertCategories, [values], (err) => {
                if (err) return res.status(500).json({ error: "Error assigning categories" });
...
app.post('/thread/:id/posts', (req, res) => {
    ...
    db.query(sql, [content, threadId, user_id], (err, result) => {
        ...
        return res.json({ message: "Post created successfully", postId: result.insertId });
    });
});
...
app.post('/post/:id/comments', (req, res) => {
    ...
    db.query(sql, [content, postId, user_id], (err, result) => {
        ...
        return res.json({ message: "Comment created successfully", commentId: result.insertId });
    });
});
...
app.post('/communities', (req, res) => {
    ...
    const sqlInsertCommunity = "INSERT INTO Community (name, description) VALUES (?, ?)";

    db.query(sqlInsertCommunity, [name, description], (err, result) => {
        if (err) return res.status(500).json({ message: "Error creating community" });

        const community_id = result.insertId;

        // SQL to add the creator to UserCommunity table
        const sqlAddCreator = "INSERT INTO UserCommunity (user_id, community_id) VALUES (?, ?)";

        db.query(sqlAddCreator, [user_id, community_id], (err, joinResult) => {
            if (err) return res.status(500).json({ message: "Error adding creator to community" });
...
app.post('/user/:id/follow', (req, res) => {
    ...
    db.query(checkSql, [followerId, followeeId], (err, result) => {
        if (err) return res.status(500).json({ message: "Error checking follow relationship" });
        if (result.length > 0) return res.status(400).json({ message: "You are already following this user" });

        // Insert the follow relationship
        db.query(insertSql, [followerId, followeeId], (err, result) => {
            if (err) return res.status(500).json({ message: "Error creating follow relationship" });
...
app.get('/user/:id/followees', (req, res) => {
    ...
    db.query(sql, [userId], (err, result) => {
        if (err) return res.status(500).json({ message: "Error fetching followees" });
        return res.json(result);
    });
});
...
app.put('/notifications/:notification_id/read', (req, res) => {
    ...
    db.query(sql, [notificationId], (err, result) => {
        if (err) return res.status(500).json({ error: "Error marking notification as read" });
...
app.post('/messages', (req, res) => {
    ...
    const sql = "INSERT INTO PrivateMessage (sender_id, receiver_id, content) VALUES (?, ?, ?)";
    db.query(sql, [sender_id, receiver_id, content], (err, result) => {
        if (err) return res.status(500).json({ error: "Error sending message." });
        return res.json({ message: "Message sent successfully!", messageId: result.insertId });
    });
});
